...
# Copyrigth & under Software IP rights (intellectual-property)
import random
import os
import csv
import io
...
class ExamSimulator:
    ...
    def curate_exam_file(self):
        # Read the CSV file
        df = pd.read_csv(questions_input_file)
        # Prepare a backup (just in case) - Murphy's Law
        df.to_csv(questions_input_file + 'bak', index=False)
        # Replace ',' with ';' and trim tabs/whitespaces in the 'long-text' columns
        df['Question'] = df['Question'].str.replace(',', ';').replace(', ',';').replace('[',' ').replace(']',' ')
        df['OptionA'] = df['OptionA'].str.replace(',', ';').replace(', ',';').replace('[',' ').replace(']',' ')
        df['OptionB'] = df['OptionB'].str.replace(',', ';').replace(', ',';').replace('[',' ').replace(']',' ')
        df['OptionC'] = df['OptionC'].str.replace(',', ';').replace(', ',';').replace('[',' ').replace(']',' ')
        df['OptionD'] = df['OptionD'].str.replace(',', ';').replace(', ',';').replace('[',' ').replace(']',' ')
        df['Explanation'] = df['Explanation'].str.replace(',', ';').replace(', ',';').replace('[',' ')

        # Save the updated CSV
        df.to_csv(questions_input_file, index=False)
        df.head()
    ...
    def load_questions(self):
        ...
        try:
            ...
            csv.field_size_limit(min(sys.maxunicode, 2147483647))  # Use 2GB as max on 32-bit systems
